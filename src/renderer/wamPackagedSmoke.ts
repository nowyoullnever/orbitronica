...
async function smokeEntry(catalogId: string, entry: typeof WAM_CATALOG[keyof typeof WAM_CATALOG], context: AudioContext, groupId: string, recorder: AudioNode) {
  ...
  if (["orbitronica-filter", "orbitronica-overdrive", "orbitronica-compressor", "orbitronica-bitcrusher", "orbitronica-flanger", "orbitronica-phaser", "orbitronica-reverb"].includes(catalogId)) await phase(catalogId, "strict-malformed-state-rejection", async () => {
